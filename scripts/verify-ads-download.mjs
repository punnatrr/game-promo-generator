...
const session=process.argv[2]||'ads-review';
...
if(!['http://localhost:3107/dashboard/ads'].includes(page))throw new Error('Open the local ads demo page first.');
...
try{
  ...
  await Promise.all([completed,command('click',process.argv[3]||'@e7')]);
  if(!filename||path.basename(filename)!==filename||!filename.endsWith('.txt'))throw new Error('Unexpected download filename');
  const bytes=await readFile(path.join(directory,filename));
  if(!bytes.toString('utf8').includes('ยังไม่ได้เผยแพร่หรือใช้เงินโฆษณา'))throw new Error('Unexpected plan export');
  const artifact=path.resolve('.test-build/ads-browser-plan.txt');
