...
export async function POST(req:NextRequest,{params}:{params:Promise<{id:string}>}) {try {const account=await user(req,true);const result=await queueRender(account.id,id((await params).id),parseRender(await body(req)));scheduleMotionWork(req);return json(result,202);}catch(error){return failure(error);}}
