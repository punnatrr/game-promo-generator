...
export async function POST(req:NextRequest,{params}:{params:Promise<{id:string}>}){try{const account=await user(req,true);const input=parseFrameRender(await body(req));const result=await queueRender(account.id,id((await params).id),{revision:input.revision,plan:{...DEFAULT_PLAN,duration:input.frame.duration}},input.frame);scheduleMotionWork(req);return json(result,202);}catch(error){return failure(error);}}
