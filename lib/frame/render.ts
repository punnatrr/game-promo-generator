...
export async function renderFrame(image:Buffer,clip:Buffer,input:FramePlan,brand:BrandProfile,logo?:Buffer){
  ...
  try{
    ...
    await exec(process.env.FFMPEG_PATH||'ffmpeg',['-hide_banner','-loglevel','error','-nostdin','-y','-filter_complex_threads','1','-ss',String(plan.start),'-protocol_whitelist','file','-format_whitelist','mov,matroska,webm','-i','clip','-loop','1','-i','frame.png','-filter_complex_script','filters.txt','-map','[out]',...(plan.audio?['-map','0:a:0?','-c:a','aac','-b:a','128k']:['-an']),'-t',String(plan.duration),'-r','24','-c:v','libx264','-threads','2','-preset','veryfast','-crf','20','-maxrate','8M','-bufsize','16M','-movflags','+faststart','output.mp4'],{cwd:directory,timeout:300000,maxBuffer:128000,windowsHide:true});
