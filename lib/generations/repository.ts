...
const SERVER_HISTORY_RETENTION_LIMIT = 50;
...
export async function recordGenerationHistory({
  ...
  const keepExistingImages = Math.max(
    0,
    SERVER_HISTORY_RETENTION_LIMIT - generatedImageCount
  );

  await pruneUserGenerationHistory({
...
async function pruneUserGenerationHistory({
  userId,
  keepImages,
}: {
  userId: string;
    ...
    where user_id = ${userId}::uuid
      and (
        ...
        or id in (
          ...
          where ranked_images.row_number > ${keepImages}
        )
      )
  `;
  ...
  await db`
    ...
    where user_id = ${userId}::uuid
      and not exists (
        select 1
        from generated_images
        where generated_images.generation_id = generations.id
      )
  `;
}
