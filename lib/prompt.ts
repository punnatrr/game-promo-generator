...
export type SpecialFeaturePrompt = {
  enabled: boolean;
  styles: SpecialFeatureStyle[];
  note?: string;
};

// Restored from production-recovery/lib/prompt.ts. Keep the current API shape,
// but do not append later special-feature instructions to the recovered prompts.
export function buildPrompt({
  targetShop,
  referenceShop = targetShop,
  aspectRatio,
}: {
  targetShop: string;
  referenceShop?: string;
  aspectRatio: string;
  specialFeature?: SpecialFeaturePrompt;
}) {
  return `
